  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 flex-1 w-full space-y-8">
        ...
        <div>
          ...
          <p className="text-slate-500 mt-2 text-lg sm:text-xl font-medium">
            Vital 144 MW baseload energy facility on the sediment-rich Kali Gandaki River
          </p>
        </div>
        ...
        <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-slate-200 bg-slate-900 group">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-900/30 to-transparent flex items-end p-6 sm:p-10">
            <div className="text-white space-y-2">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold">Beltari, Syangja, Gandaki Province</h2>
              <p className="text-slate-300 text-sm sm:text-base max-w-2xl">
                Situated on a major loop of the sacred Kali Gandaki River, utilizing state-of-the-art desilting infrastructure.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
            ...
            <span className="text-2xl sm:text-4xl font-extrabold text-slate-900">144 <span className="text-lg font-bold text-red-600">MW</span></span>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
            <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block mb-1">Annual Generation</span>
            <span className="text-2xl sm:text-4xl font-extrabold text-slate-900">840 <span className="text-lg font-bold text-blue-600">GWh</span></span>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
            <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block mb-1">Turbine Units</span>
            <span className="text-2xl sm:text-4xl font-extrabold text-slate-900">3 × 48 <span className="text-lg font-bold text-slate-600">MW</span></span>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
            <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block mb-1">Commission Year</span>
            <span className="text-2xl sm:text-4xl font-extrabold text-slate-900">2002</span>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-xs space-y-4">
              ...
              <p className="text-slate-600 leading-relaxed">
                The Kali Gandaki A Hydropower Project is one of Nepal's largest operating run-of-river plants, owned and operated by the Nepal Electricity Authority (NEA). Built with assistance from the Asian Development Bank (ADB) and Japan Bank for International Cooperation (JBIC), it was commissioned in 2002 to address severe national energy deficits.
              </p>
              <p className="text-slate-600 leading-relaxed">
                The plant features a concrete gravity diversion dam across the Kali Gandaki River at Mirmi. A unique engineering highlight is its large-scale desilting basin complex designed to filter heavy Himalayan quartz silt load before directing water into the 5.9-kilometer headrace tunnel.
              </p>
              <p className="text-slate-600 leading-relaxed">
                Water is delivered down penstock pipes to an underground powerhouse located downstream at Beltari, housing three 48 MW Francis turbine generators operating under a rated gross head of 115 meters.
              </p>
            </div>
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-xs space-y-4">
              <h3 className="text-2xl font-bold text-slate-900 border-b border-slate-100 pb-4">
                Engineering Highlights & Innovation
              </h3>
              <ul className="space-y-3 text-slate-600">
                <li className="flex items-start gap-3">
                  ...
                  <span><strong>Sediment Handling:</strong> Features 2 underground desilting basins with flushing systems to protect Francis turbine runners from erosion.</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="w-5 h-5 bg-red-100 text-red-600 rounded-full flex items-center justify-center font-bold text-xs mt-0.5">✓</span>
                  <span><strong>Subterranean Powerhouse:</strong> Built deep inside stable rock strata to withstand seismic forces and extreme mountain terrain.</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="w-5 h-5 bg-red-100 text-red-600 rounded-full flex items-center justify-center font-bold text-xs mt-0.5">✓</span>
                  <span><strong>Integrated Transmission:</strong> Feeds power directly into Pokhara and Butwal 132 kV grid corridors.</span>
                </li>
              </ul>
            </div>
          </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-4">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between py-2 border-b border-slate-100">
                  ...
                  <span className="font-bold text-slate-800">Francis (3 Units)</span>
                </div>
                <div className="flex justify-between py-2 border-b border-slate-100">
                  <span className="text-slate-500 font-medium">Design Discharge</span>
                  <span className="font-bold text-slate-800">141 m³/s</span>
                </div>
                <div className="flex justify-between py-2 border-b border-slate-100">
                  <span className="text-slate-500 font-medium">Gross Head</span>
                  <span className="font-bold text-slate-800">115 m</span>
                </div>
                <div className="flex justify-between py-2 border-b border-slate-100">
                  <span className="text-slate-500 font-medium">Headrace Tunnel</span>
                  <span className="font-bold text-slate-800">5.9 km</span>
                </div>
                <div className="flex justify-between py-2 border-b border-slate-100">
                  <span className="text-slate-500 font-medium">Dam Type</span>
                  <span className="font-bold text-slate-800">Concrete Gravity (44 m high)</span>
                </div>
                <div className="flex justify-between py-2 border-b border-slate-100">
                  <span className="text-slate-500 font-medium">Transmission</span>
                  <span className="font-bold text-slate-800">132 kV lines</span>
                </div>
      ...
      <footer className="bg-white border-t border-slate-200 py-6 text-center text-xs text-slate-400">
        Nepal Hydropower Infrastructure Project • Kali Gandaki A Hydropower (144 MW)
      </footer>
    </div>
  );
};
